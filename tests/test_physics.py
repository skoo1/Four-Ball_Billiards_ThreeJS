...
class TestSquirt:
    ...
    def test_right_english_deflects_left(self):
        """우측 당점(offset_x > 0) → 큐 기준 좌측 편향 (vx > 0 at azimuth=0)."""
        ball = Ball("test")
        PhysicsEngine.apply_cue(
            ball,
            force=3.0,
        ...
        assert ball.velocity[0] > 0, (
            f"Right english → left deflection (cue frame): vx={ball.velocity[0]:.6f}"
        )

    def test_left_english_deflects_right(self):
        """좌측 당점(offset_x < 0) → 큐 기준 우측 편향 (vx < 0 at azimuth=0)."""
        ball = Ball("test")
        PhysicsEngine.apply_cue(
            ball,
            force=3.0,
        ...
        assert ball.velocity[0] < 0, (
            f"Left english → right deflection (cue frame): vx={ball.velocity[0]:.6f}"
        )
...
class TestRightHandRule:
    """apply_cue의 직교 좌표 프레임이 오른손 법칙(Right-hand rule)을 따르는지 검증.

    이 테스트는 외적 순서 오류(cross(up,fwd) vs cross(fwd,up))를 근본적으로 방지한다.
    """

    def test_right_vector_facing_forward(self):
        """direction=[0,0,1] (전방)일 때 right = cross(fwd, up) = [-1,0,0]."""
        ...
        np.testing.assert_allclose(right, [-1.0, 0.0, 0.0], atol=1e-12,
            err_msg="direction=[0,0,1]일 때 right는 [-1,0,0]이어야 함 (오른손 법칙)")

    def test_right_vector_facing_backward(self):
        """direction=[0,0,-1] (후방)일 때 right = cross(fwd, up) = [1,0,0]."""
        fwd = np.array([0.0, 0.0, -1.0])
        up = np.array([0.0, 1.0, 0.0])
        right = np.cross(fwd, up)
        right = right / np.linalg.norm(right)
    ...
    def test_local_up_from_right_cross_fwd(self):
        """right × fwd = local_up 이 [0,1,0] 방향이어야 함 (세 축 직교 검증)."""
        for fwd_dir in [[0, 0, 1], [0, 0, -1], [1, 0, 0], [-1, 0, 0],
                         [0.6, 0, 0.8], [-0.5, 0, 0.5]]:
            fwd = np.array(fwd_dir, dtype=float)
            fwd = fwd / np.linalg.norm(fwd)
            up = np.array([0.0, 1.0, 0.0])

            right = np.cross(fwd, up)
            right = right / np.linalg.norm(right)
            local_up = np.cross(right, fwd)
            local_up = local_up / np.linalg.norm(local_up)

            # local_up의 y 성분은 양수(위쪽)여야 함
            assert local_up[1] > 0.99, (
                f"fwd={fwd_dir}: local_up={local_up} — y 성분이 양수여야 함"
            )

    def test_orthogonality(self):
        """fwd, right, local_up 세 벡터가 서로 직교해야 함."""
        for fwd_dir in [[0, 0, 1], [0, 0, -1], [0.6, 0, 1.0], [-1, 0, 0.3]]:
            fwd = np.array(fwd_dir, dtype=float)
            fwd = fwd / np.linalg.norm(fwd)
            up = np.array([0.0, 1.0, 0.0])
            ...
            assert abs(np.dot(fwd, right)) < 1e-12, (
                f"fwd·right={np.dot(fwd, right):.2e} — 직교 위반"
            )
            assert abs(np.dot(fwd, local_up)) < 1e-12, (
                f"fwd·local_up={np.dot(fwd, local_up):.2e} — 직교 위반"
            )
            assert abs(np.dot(right, local_up)) < 1e-12, (
                f"right·local_up={np.dot(right, local_up):.2e} — 직교 위반"
            )

    def test_apply_cue_uses_correct_right_vector(self):
        """apply_cue 내부에서 실제로 올바른 right 벡터를 사용하는지 통합 검증.

        direction=[0,0,1], offset=[+x, 0] (우측 당점) →
        당점이 공의 오른쪽(=-X 방향)에 위치 → 수구에 +ωy (CCW from above) 전달.
        """
        ball = Ball("test")
        PhysicsEngine.apply_cue(
            ball,
            force=3.0,
            direction=np.array([0.0, 0.0, 1.0]),
            offset=np.array([0.02, 0.0]),  # 우측 당점
        )
        # 우측 당점 → 시계 방향 회전(위에서 보면) → wy > 0 (Y-up 오른손 법칙)
        assert ball.angular_velocity[1] > 0, (
            f"우측 당점 → wy > 0 (CCW from above): wy={ball.angular_velocity[1]:.6f}"
        )

    def test_squirt_factor_positive(self):
        """SQUIRT_FACTOR가 양수인지 확인 — 음수는 외적 순서 오류 보상 꼼수."""
        assert SQUIRT_FACTOR > 0, (
            f"SQUIRT_FACTOR={SQUIRT_FACTOR} — 양수여야 함. "
            "음수는 잘못된 right 벡터를 보상하기 위한 triple sign error"
        )
