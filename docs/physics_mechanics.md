...
| 상수 | 변수명 | 값 | 단위 | 설명 |
|---|---|---|---|---|
| 반지름 | `R` | 0.03275 | m | 국제 4구 규격 (65.5mm) |
| 질량 | `m` | 0.21 | kg | 표준 중량 |
...
$$I = \frac{2}{5} \times 0.21 \times 0.03275^2 \approx 9.01 \times 10^{-5} \ \text{kg·m}^2$$
...
큐의 진행 방향 $\hat{\mathbf{f}}$ (정규화), 위 벡터 $\hat{\mathbf{u}} = (0,1,0)$, 오른쪽 벡터 $\hat{\mathbf{r}} = \hat{\mathbf{f}} \times \hat{\mathbf{u}}$, 로컬 위 벡터 $\hat{\mathbf{u}}_{local} = \hat{\mathbf{r}} \times \hat{\mathbf{f}}$:

> **주의 (오른손 법칙)**: `cross(forward, up) = right`. `cross(up, forward)`는 **Left** 벡터를 산출하므로 절대 사용하지 말 것. 예: $\hat{\mathbf{f}} = (0,0,1)$ 일 때 $\hat{\mathbf{r}} = (0,0,1) \times (0,1,0) = (-1,0,0)$ ✓

$$\mathbf{c} = \hat{\mathbf{r}} \cdot o_x + \hat{\mathbf{u}}_{local} \cdot o_y + \hat{\mathbf{f}} \cdot o_z \quad \text{(월드 좌표의 접촉 벡터)}$$
...
### 3.3 스쿼트 (Squirt / Deflection)

좌우 당점(English) 타격 시, 공은 큐의 조준 방향과 약간 다른 방향으로 튀어나간다. 이를 **스쿼트(squirt)** 라 한다.

타점 수평 오프셋 $o_x$ 에 의한 편향각:

$$\theta_{sq} = \text{SQUIRT\_FACTOR} \cdot \arctan\left(\frac{o_x}{R}\right)$$

> **중요**: `SQUIRT_FACTOR`는 **양수** (기본값 0.02). 우측 당점($o_x > 0$) → 편향각 양수 → 공이 큐 기준 좌측으로 편향. 음수 값은 외적 순서 오류의 보상 꼼수이므로 절대 사용 금지.
...
| 우측 (Right English) | +값 | 0 | 사이드스핀 (ω_y > 0) + 좌측 스쿼트 편향 |
| 좌측 (Left English) | -값 | 0 | 사이드스핀 (ω_y < 0) + 우측 스쿼트 편향 |
...
### 10.5 접선 마찰 임펄스 — Throw 효과 (Coulomb 모델)

빗맞음 충돌에서 접촉면의 **접선 방향 미끄럼**이 있으면, Coulomb 마찰에 의해 접선 임펄스가 발생한다. 이것이 당구의 **Throw** 효과(분리각 편향)와 **스핀 전달**의 원인이다.

**접촉 벡터**:

$$\mathbf{r}_{c1} = -R\hat{\mathbf{n}}, \qquad \mathbf{r}_{c2} = +R\hat{\mathbf{n}}$$

**접촉점 미끄럼 속도**:

$$\mathbf{v}_{slip} = (\mathbf{v}_1 + \boldsymbol{\omega}_1 \times \mathbf{r}_{c1}) - (\mathbf{v}_2 + \boldsymbol{\omega}_2 \times \mathbf{r}_{c2})$$

접선 성분만 추출:

$$\mathbf{v}_{slip,t} = \mathbf{v}_{slip} - (\mathbf{v}_{slip} \cdot \hat{\mathbf{n}})\hat{\mathbf{n}}$$

**유효 접선 질량**: 동질량 구 두 개의 회전+병진 관성 합산:

$$\frac{1}{m_{eff,t}} = \frac{1}{m} + \frac{R^2}{I} + \frac{1}{m} + \frac{R^2}{I} = \frac{2}{m} + \frac{2 \times 2.5}{m} = \frac{7}{m}$$

$$m_{eff,t} = \frac{m}{7}$$

**Coulomb 마찰 판정**:

$$J_{stop} = |\mathbf{v}_{slip,t}| \cdot m_{eff,t} \qquad \text{(미끄럼 완전 정지에 필요한 임펄스)}$$

$$J_{max} = \mu_{ball} \cdot j \qquad \text{(법선 임펄스 }j\text{에 비례하는 Coulomb 한계)}$$

$$J_t = \min(J_{stop}, J_{max})$$

**속도 및 각속도 업데이트**:

$$\hat{\mathbf{t}} = \frac{\mathbf{v}_{slip,t}}{|\mathbf{v}_{slip,t}|}, \qquad \mathbf{J}_{tang} = -\hat{\mathbf{t}} \cdot J_t$$

$$\mathbf{v}_1' \mathrel{+}= \frac{\mathbf{J}_{tang}}{m}, \qquad \mathbf{v}_2' \mathrel{-}= \frac{\mathbf{J}_{tang}}{m}$$

$$\Delta\boldsymbol{\omega} = \frac{\mathbf{r}_{c1} \times \mathbf{J}_{tang}}{I}$$

$$\boldsymbol{\omega}_1' \mathrel{+}= \Delta\boldsymbol{\omega}, \qquad \boldsymbol{\omega}_2' \mathrel{+}= \Delta\boldsymbol{\omega}$$

> **참고**: 두 공의 각속도 변화가 동일 부호인 이유는, $\mathbf{r}_{c1} = -\mathbf{r}_{c2}$ 이고 $\mathbf{J}_{tang}$ 의 부호도 반대가 되어 외적 결과가 같아지기 때문이다.

`MU_BALL = 0.08` (기본값). 이 값을 0으로 설정하면 throw 없이 기하학적 분리각만 남는다.
...
### 11.6 쿠션 접촉 높이 오프셋 (RAIL_H_OFFSET)

실제 당구대의 쿠션은 공의 정확한 중심 높이가 아닌, 중심보다 약간 위에서 접촉한다. 이 높이 차이가 **법선 충돌 임펄스에 의한 추가 토크**를 생성한다.

접촉 높이 오프셋: $h$ = `RAIL_H_OFFSET` = 0.003 m

접촉점의 수평 반경 (기하학):

$$R_{xy} = \sqrt{R^2 - h^2}$$

**법선 임펄스에 의한 토크**: 접촉점이 공 중심보다 높이 $h$ 에 있으므로, 법선 방향 충돌 임펄스 $J_n$ 이 수직축(ω_x 또는 ω_z) 주위로 토크를 발생시킨다.

| 쿠션 | 법선 토크 효과 |
|---|---|
| 우쿠션 (x_max) | $\Delta\omega_z = +h \cdot J_n / I$ |
| 좌쿠션 (x_min) | $\Delta\omega_z = -h \cdot J_n / I$ |
| 앞쿠션 (z_max) | $\Delta\omega_x = -h \cdot J_n / I$ |
| 뒤쿠션 (z_min) | $\Delta\omega_x = +h \cdot J_n / I$ |

이 효과의 물리적 의미: 쿠션에 부딪힌 공이 반사 후 약간의 탑스핀/백스핀을 얻는다. $h$ 값이 클수록 멀티 쿠션 후 스핀이 더 크게 축적된다.

> **주의**: $h$ 가 과도하면 (> 0.005) 3쿠션 이상에서 비현실적 스핀 축적이 발생한다. 실제 카롬 당구 쿠션 접촉 높이(~2.25–4.25mm)에 맞추어 0.003m를 사용한다.

### 11.7 잉글리시(English) 효과의 물리적 해석
...
| 구름 마찰 계수 | `MU_ROLL` | 0.015 | 무차원 | 구름 감속 강도. 공이 멈추는 거리 결정 |
| 수직 스핀 감쇠 계수 | `MU_SPIN` | 0.04 | 무차원 | 피벗 마찰. 잉글리시 지속 시간 결정 |
| 쿠션 반발 계수 | `CUSHION_RESTITUTION` | 0.75 | 무차원 | 쿠션 충돌 후 법선 속도 비율 |
| 공-공 반발 계수 | `BALL_RESTITUTION` | 0.94 | 무차원 | 두 공 충돌 후 상대속도 비율 |
| 중력 가속도 | `GRAVITY` | 9.8 | m/s² | 마찰력 크기와 점프 궤적에 영향 |
| 쿠션 마찰 계수 | `MU_CUSHION` | 0.18 | 무차원 | 쿠션 접선 Coulomb 마찰. 잉글리시 쿠션 효과 결정 |
| 공-공 마찰 계수 | `MU_BALL` | 0.08 | 무차원 | 공-공 Coulomb 마찰. Throw 효과(분리각 편향) 결정 |
| 스쿼트 계수 | `SQUIRT_FACTOR` | 0.02 | 무차원 | 좌우 당점 시 초기 속도 편향 크기 |
| 쿠션 접촉 높이 | `RAIL_H_OFFSET` | 0.003 | m | 공 중심 위 쿠션 접촉점 높이. 쿠션 법선 토크 결정 |
...
**`MU_BALL`** 을 0으로 설정하면:
- 공-공 충돌에서 기하학적 분리각만 적용 (throw 없음)
- 빗맞음 충돌 후 스핀 전달 없음

**`RAIL_H_OFFSET`** 을 0으로 설정하면:
- 쿠션에서 법선 토크 발생 없음 → 반사 후 추가 스핀 없음
- 멀티 쿠션에서 현실감 감소

**`CUSHION_RESTITUTION`** vs **`BALL_RESTITUTION`**:
- 쿠션은 고무+천 구조로 에너지 손실이 크다 (0.75)
- 당구공은 경질 페놀 수지로 에너지 손실이 매우 작다 (0.94)
...
*최종 수정: 2026-02-22*
*대응 코드: `physics.py` (commit e0c4b28)*
